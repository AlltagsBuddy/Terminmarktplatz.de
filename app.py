...
def redirect_www():
    ...
    if host.startswith('www.terminmarktplatz.de'):
        # leite auf die gleiche Route/Query der kanonischen Domain
        path = request.full_path
        # Flask hängt bei full_path immer ein ? an, selbst ohne Query → aufräumen
        if path.endswith('?'):
            path = path[:-1]
...
def page_home_canonical():
    # Optional: /index.html auf / umleiten
...
@app.get('/portal')
def page_portal():
    # Voraussetzung: templates/portal.html existiert
...
def register():
    ...
    with Session(engine) as s:
        ...
        s.commit()
        payload = {
            'sub': p.id, 'aud': 'verify', 'iss': JWT_ISS,
...
def auth_login():
    ...
    resp.set_cookie('access_token', access, max_age=JWT_EXP_MIN*60,
                    httponly=True, secure=True, samesite='Lax')
    resp.set_cookie('refresh_token', refresh, max_age=REFRESH_EXP_DAYS*86400,
                    httponly=True, secure=True, samesite='Lax')
    return resp
...
def admin_slots():
    ...
    with Session(engine) as s:
        items = s.scalars(
            select(Slot).where(Slot.status == status).order_by(Slot.start_at.asc())
        ).all()
